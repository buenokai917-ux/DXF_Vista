...
import { renderDxfToCanvas, aciToHex } from './utils/renderUtils';
...
// Standard CAD Colors for UI Picker
const CAD_COLORS = [
  '#FF0000', // Red
  '#FFFF00', // Yellow
  '#00FF00', // Green
    ...
    const processFiles = async () => {
      ...
      try {
        await Promise.all(fileList.map(async (file) => {
          ...
          // Determine colors for new layers based on DXF Layer Table (ACI)
          parsed.layers.forEach((layerName) => {
            if (newColors[layerName]) return; // Skip if already colored

            const layerDef = parsed.layerDictionary[layerName];
            let aci = 7; // Default White/Black
            if (layerDef && layerDef.color !== undefined) {
              aci = layerDef.color;
            }
            newColors[layerName] = aciToHex(aci);
          });
      ...
      >
        ...
        <div className="flex border-b border-slate-800 shrink-0">
          <button
            ...
            className={`flex-1 py-3 text-xs font-medium uppercase tracking-wider transition-colors ${sidebarTab === 'LAYERS'
          ...
          <button
            ...
            className={`flex-1 py-3 text-xs font-medium uppercase tracking-wider transition-colors ${sidebarTab === 'ANALYSIS'
              ? 'text-blue-400 border-b-2 border-blue-400 bg-slate-800/50'
              : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'
              }`}
          >
                ...
                <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
                  ...
                  {filteredLayers.map((layer) => (
                    <div key={layer} className="flex flex-col mb-0.5">
                      <div
                        className={`flex items-center p-2 rounded transition-colors group ${activeProject.activeLayers.has(layer) ? 'bg-slate-800 text-slate-200' : 'text-slate-500 hover:bg-slate-800/50'
                          }`}
                      >
                        <div
